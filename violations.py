...
class CollisionViolation(Violation):
    def __init__(self, args: Dict):
        ...
        assert "joint_ids" in args, "Missing required arg from: {}".format(
            required_args
        )
        assert "object_id_self" in args, "Missing required arg from: {}".format(
            required_args
        )
        assert "object_ids_env" in args, "Missing required arg from: {}".format(
            required_args
        )
        assert (
            type(args["joint_ids"]) == list
        ), "Invalid argument type: joint_ids must be a list"
        assert (
            type(args["object_ids_env"]) == list
        ), "Invalid argument type: object_ids_env must be a list"
        assert (
            type(args["object_id_self"]) == int
        ), "Invalid argument type: object_id_self must be a float"
    ...
    def _find_collisions(self, joint_states: List):
        ...
        current_joint_states = p.getJointStates(
            self.self_id, range(p.getNumJoints(self.self_id))
        )
